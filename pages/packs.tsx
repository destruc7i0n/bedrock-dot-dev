        ...
        <div className='max-w-screen-xl mx-auto px-4 sm:px-6 md:px-8 mt-8 mb-10'>
          <div className='max-w-screen-lg mx-auto space-y-4'>
            ...
            <p className='text-lg text-black dark:text-gray-200'>
              <Trans i18nKey='page.packs.subtitle' t={t} components={[
                <a className='link' href='https://github.com/bedrock-dot-dev/packs' target='_blank' />,
                <a className='link' href='https://github.com/Mojang/bedrock-samples?ref=bedrock.dev' target='_blank' />,
              ]} />
            </p>

            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 font-normal'>
              {versionsSorted.map((v, i) => <PackCard key={`packs-versions-${i}`} versionName={v} versionData={versions[v]} />)}
            </div>
          </div>
        </div>
...
export const getStaticProps: GetStaticProps = async ({ locale: localeVal }) => {
  ...
  for (const release of mojangBedrockSamples) {
    ...
    if (version && !versions[version]) versions[version] = { git: release.html_url }
  }
