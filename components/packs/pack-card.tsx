...
const PackCard: FunctionComponent<PackCardProps> = ({ versionName, versionData }) => {
  ...
  const s3Links = (
    <>
      {versionData.b ? (
        <a href={getUrl('behaviours', versionName)} className='link' download>{t('component.packs_page.behaviours_link')}</a>
      ) : (
        <p className='text-gray-500 dark:text-gray-400'>{t('component.packs_page.behaviours_link')}</p>
      )}

      {versionData.r ? (
        <a href={getUrl('resources', versionName)} className='link' target='_blank' download rel='noopener'>{t('component.packs_page.resources_link')}</a>
      ) : (
  ...
  const githubLink = (
    <a href={versionData.git} className='link' target='_blank' rel='noopener'>GitHub</a>
  )
  ...
  const content = isGitHubLink
    ...
    : (
      open
        ? s3Links
        : <span className='link cursor-pointer' onClick={() => setOpen(!open)}><ArrowDownTrayIcon className='pointer-events-none w-6 h-6' /></span>
    )
